    ...
    IEnumerator SolitaireDeal()
    {
        for (int i = 0; i < 7; i++)
        {
            ...
            foreach (string card in bottoms[i])
            {
                ...
                {
                    GameObject newCard = Instantiate(cardPrefab, new Vector3(bottomPos[i].transform.position.x, bottomPos[i].transform.position.y - yOffset, bottomPos[i].transform.position.z - zOffset),
                ...
                else
                {
                    GameObject newCard = Instantiate(cardPrefab, new Vector3(bottomPos[i].transform.position.x, bottomPos[i].transform.position.y - yOffset, bottomPos[i].transform.position.z - zOffset),
                    Quaternion.identity, bottomPos[i].transform);
                    //newCard.transform.localScale *= 0.4f;
                    newCard.name = card;
                    newCard.GetComponent<Selectable>().row = i;
                }
