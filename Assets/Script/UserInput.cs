...
public class UserInput : MonoBehaviour
{
    ...
    private float timer;
    private float doubleClikTime = 0.3f;
    ...
    void Card(GameObject selected)
    {
        ...
        else if (selected.GetComponent<Selectable>().inDeckPile)//if the card clicked on is in the deck pile with the trips
        {
            ...
            if (!Blocked(selected))
            {
                if(slot1 == selected) // if the same card click is twice
                {
                    if (DoubleClick())
                    {
                        //attempt auto back
        ...
        else if (slot1 == selected) // if the same card click is twice
        {
            if (DoubleClick())
            {
                //attempt auto back
    ...
    void Top(GameObject selected)
    {
        ...
        {
            // if the card is an ace and the empty  slot is topthen stack
    ...
    bool Stackable(GameObject selected)
    {
        ...
        if (!s2.inDeckPile)
        {
            ...
            else //if in the bottom pile must stack alternate colour king to ace
            {
                if (s1.value == s2.value - 1)
                {
                    bool card1Red = true;
    ...
    void Stack(GameObject selected)
    {
        ...
        slot1.transform.parent = selected.transform; //this makes children move with the parent
        ...
        else if (s1.top && s2.top && s1.value == 1) // allows movement of cards between topspots
        {
        ...
        else // remove the card string from the approprriate bottom list
        {
    ...
    bool Blocked(GameObject selected)
    {
        ...
        {
            if(s2.name == solitaire.tripsOnDisplay.Last()) // if it is the last tripis not blocked
            {
                return false;
            }
            else
    ...
    bool DoubleClick()
    {
        if(timer < doubleClikTime && clickCount == 2)
        {
            print("Double Click");
            return true;
        }
    ...
    void AutoStack(GameObject selected)
    {
        for(int i = 0; i < solitaire.topPos.Length; i++)
        {
            ...
            if(selected.GetComponent<Selectable>().value == 1) // if it is Ace  
            {
